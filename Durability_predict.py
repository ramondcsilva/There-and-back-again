# -*- coding: utf-8 -*-
"""
Created on Mon Aug 26 20:40:43 2019
...
classe = result.iloc[:,14].values
# Exclui o mesmo da base de dados previsora
result = result.drop(columns = 14)
...
plt.plot(fprTREE, tprTREE, '', label="Durability, AUC= %0.2f"% aucTREE)
...
# Uso de biblioteca para Plotagem de Gráfico
plt.plot(fprNB, tprNB, '', label="Durability, auc= %0.2f"% aucNB)
plt.title('Receiver Operating Characteristic')
plt.xlabel('False Positive')
plt.ylabel('True Positive')
plt.legend(loc=4)
...
'''
#criando uma ensemble de AdaBoost com 30 árvores de decisão
classificadorAdaBoost = AdaBoostClassifier(tree, n_estimators = 30, learning_rate = 1)
